...
/// Portable mode support for Praat.
///
...
pub fn init() {
    PORTABLE_DATA_DIR.get_or_init(|| {
        ...
        let is_portable = if is_valid_portable_marker(&marker_path) {
            ...
            let _ = std::fs::write(&marker_path, "Praat Portable Mode");
            true
        } else {
            false
        };
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| s.trim().starts_with("Praat Portable Mode"))
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        ...
        write!(f, "Praat Portable Mode").unwrap();
